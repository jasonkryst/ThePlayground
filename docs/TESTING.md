...
- **Known-benign act() warnings (issues #147/#165):** `useGameSession` and `useMemorySession` each poll a display-only `currentElapsedMs` via a genuine wall-clock `setInterval(...,100)` (deliberately unconditional — see the comment on each hook's timer effect). In any `userEvent`-driven component test that reaches real gameplay, that tick can land in the real-time gap between two sequential `act()` calls no matter how the test awaits things, since fake timers can't be combined with `userEvent` here (previous point). `src/test-setup.js` filters `console.error` for this one specific, stack-verified case via `src/lib/suppressKnownActWarnings.js` (`isKnownBenignActWarning`) — it only drops a warning whose update genuinely traces back to one of those two hooks' interval callbacks, so any other act() warning (including these same message strings from a different source) still prints and fails a review the same as before. Don't add ad hoc per-test timing waits to chase this warning — it's already handled centrally, and per-test real-timer waits were tried and found unreliable (the exact `act()` boundary the tick lands in varies run to run).
...
- **Storage-adapter contract:** `src/storage/__tests__/adapterContract.js` exports `runAdapterContractTests(adapterFactory, { label })`, an adapter-agnostic suite asserting the original five get/save pairs documented in `src/storage/adapter.js` (the newer `getItemStats`/`saveItemStats` and session-resume methods aren't yet covered by this contract suite) (default/empty shapes, round-trips, key isolation). Any adapter's own test file calls it in one line — see `localStorageAdapter.contract.test.js`. It's separate from `localStorageAdapter.*.test.js`, which cover localStorage-specific behavior (corrupt-JSON recovery, migrations) out of the contract's scope.
...
**Design-token duplication (not a contrast issue, a maintainability one):** `src/__tests__/designTokenColors.test.js` (issue #146) is a different kind of check from the two above — it doesn't compute contrast, it guards against a hardcoded hex literal creeping back in wherever CLAUDE.md's design-token convention already applies. It reads the relevant `.css` files as plain text (same `fs.readFileSync` pattern as `nginx/__tests__/securityHeaders.test.js`) and asserts both that the shared token (`--color-success`) is referenced via `var(...)`, and that the specific literals it replaced (`#a5d6a7`, the stale `#666` `var()` fallback) are gone.

**`aria-hidden` blind spot:** axe (both levels) excludes `aria-hidden` elements from its contrast scan, since they're not exposed to assistive tech — but a sighted user still sees them. A decorative icon glyph that's a plain-text character (not a color emoji) inherits CSS `color` and can still render invisibly against its own background without axe ever flagging it (issue #152: the header's theme-toggle icon). There's no general automated guard for this class of bug — when adding a decorative `aria-hidden` icon that isn't a color emoji, explicitly verify (or add a targeted `getComputedStyle` check like the ones in `e2e/themes.spec.js`'s "theme toggle icon visibility" block) that its color isn't equal to its background across every theme.
...
Playwright starts both `npm run dev` (port 5173) and Storybook (port 6006) automatically via the `webServer` array in `playwright.config.js`. `globalSetup` (`e2e/global-setup.js`) then visits every route once, single-threaded, before the test workers start, so each route's module graph is already compiled by the time real tests begin rather than paying that cost mid-suite (issue #141).

**CI runs the whole `e2e` suite on a single Playwright worker** (`workers: process.env.CI ? 1 : undefined` in `playwright.config.js`; local dev keeps Playwright's own default parallelism). This isn't a throughput choice — `ubuntu-latest`'s ~2 vCPUs couldn't reliably run 2 workers' Chromium instances concurrently against one shared `npm run dev` process: whichever test happened to be rendering when the other worker's browser stole CPU time could stall past even a 60s timeout, and *which* test lost that race was effectively random. It surfaced three separate times in three unrelated tests — `confetti-csp.spec.js` (#141), `admin.spec.js`'s resume-prompt flow (#147/#165), then `dashboard.spec.js` and `css-validity.spec.js` again on the very next CI run after #165's fix landed (#167) — because point-patching the specific test each time never removed the actual contention. `retries: 0` stays as-is (deliberate — see `e2e/global-setup.js`'s comment: a flaky pass-on-retry is treated as worse than a loud failure); single-worker CI removes the need for it here instead of masking it. The specs in `e2e/`:

**Guard every navigation immediately after an action that persists to storage.** `admin.spec.js`'s "replay intro brings back a dismissed game intro" test (issues #147/#165) flaked from a snapshot-persist/navigation race: clicking the resume prompt's "Start Fresh" starts a brand-new session whose resumable snapshot needs two React effect passes (queue-build, then snapshot-save) before it lands in `localStorage` — on a fast/idle machine both passes finish before the test's next `page.goto()` (a real navigation, unlike an in-app client-side route change), but under load they don't always, so the next visit found no snapshot and fell through to the intro instead of the resume prompt. The test now `expect.poll()`s `localStorage`'s `playground_session_resume` key before navigating away, matching the file's two other pre-existing guards for the same class of hazard; `useGameSession.js` was also hardened to persist a freshly built queue's snapshot from refs in the same effect that builds it, rather than waiting for a second render pass. When adding a new navigation right after an action whose effect is only observable via storage, poll for that storage state first — don't assume same-tick persistence.
...
| `pwa.spec.js` | PWA support (issue #96): the manifest is valid and installable, its icons and the apple-touch-icon resolve, the previously-dead `/vite.svg` favicon reference is gone and replaced with a working one, and — the core claim — the service worker registers, activates, and a previously-visited page survives going fully offline; a negative control proves a *fresh* context with no installed service worker does NOT survive offline, so the positive case is actually exercising the cache and not a browser fluke. Runs against a real `npm run build` + `vite preview` (not the dev server, which never gets the manifest/SW injection at all — see below), so it needs no Docker |
| `pwa-csp.spec.js` | The service worker/manifest specifically under this app's real, locked-down CSP (issue #96): registers/activates and survives offline the same as `pwa.spec.js`, plus a negative check that no CSP violations are logged during registration. Boots the same live nginx container as `nginx-headers.spec.js`/`confetti-csp.spec.js`; skips without Docker — see below |
...
| `zoom-large-text.spec.js` | 200%-zoom-equivalent viewport overflow checks, OS/browser large-text scaling of quiz/memory/base text (px→rem conversion), the sticky-header-obscures-focused-memory-tile fix (issue #83), and chart-axis/heatmap alignment under large text (issue #130) |
| `session-resume.spec.js` | Session-resume flow (issue #128): a mid-session exit/reload leaves a resumable snapshot, the resume prompt offers to continue or start fresh within the 4-hour window, and the snapshot expires/clears correctly. Also covers issue #153: the resumed question's audio must not autoplay while the resume prompt is still awaiting a decision, and must play once Resume/Start Fresh is chosen — tracked via a real `HTMLMediaElement.play()` count, not a mock |
...
| `themes.spec.js` | Light/Dark/High Contrast: axe-core scans (including `color-contrast`) of Dashboard/Admin/a game's results screen under each theme, `system` resolving correctly under both `prefers-color-scheme` values via `page.emulateMedia`, the header theme-toggle's click-cycle + persistence across reload, and (issue #152) direct `getComputedStyle` checks that the theme-toggle icon's color isn't the same as the header background in every theme, including an invalid persisted theme value |

**`nginx-headers.spec.js`, `confetti-csp.spec.js`, and `pwa-csp.spec.js` are the specs that aren't browser tests against the dev server** — the app's `vite dev` server never applies `nginx.conf` (or its CSP) at all, so the only way to prove the shipped config actually behaves correctly is to run it in real nginx. `nginx-headers.spec.js` boots the same pinned `nginxinc/nginx-unprivileged:1.31-alpine` image the Dockerfile ships (mounting `nginx.conf`, `nginx/security-headers.conf`, and a handful of fixture asset files — not the full `Dockerfile` build, which needs `npm run build` first and is much slower) and drives it with Playwright's `request` fixture. `confetti-csp.spec.js` boots the same image but *does* run `npm run build` first and mounts the real `dist/`, then drives an actual `page` through a memory-match play-through: `canvas-confetti`'s default export tries to build its animation loop from a `blob:` Web Worker, which this app's CSP silently kills (no `worker-src`, and `script-src` doesn't allow `blob:`) — that's exactly what broke confetti/fireworks in every real deployment despite the setting being on, and only a real browser enforcing the real CSP can prove the fix (`src/lib/confetti.js` forcing main-thread rendering via `create(null, { useWorker: false })`) actually renders pixels again. `pwa-csp.spec.js` boots the same image the same way, to prove the service worker registers and the manifest fetches cleanly under the real CSP rather than the no-header dev/preview environment. All three require a running Docker daemon and `test.skip()` with a clear reason when it isn't available, so `npm run e2e` still runs everywhere else. A companion fast check with no Docker dependency, `nginx/__tests__/securityHeaders.test.js` (runs under `npm test`), statically parses `nginx.conf` and fails if any `location` block sets its own `add_header` without including the shared security-headers snippet — this is what actually catches a headers regression on a machine without Docker; `src/lib/__tests__/confetti.test.js` is the equivalent fast, Docker-free guard for the confetti fix, asserting (with a mocked `canvas-confetti`) that `create()` is always called with `useWorker: false`.

**`pwa.spec.js` is the one spec that runs against neither the dev server nor Docker** — `vite-plugin-pwa` only injects the manifest link, icon links, and service-worker registration script into the *built* `index.html` (`devOptions.enabled` is deliberately left off, since a dev-mode service worker is a common source of confusing stale-cache bugs while iterating), so testing against `npm run dev` would prove nothing. It instead runs its own `npm run build` + `vite preview` in `beforeAll`/`afterAll`, the same self-contained pattern as the Docker specs but without needing Docker, since CSP enforcement isn't what it's proving — that's `pwa-csp.spec.js`'s job.
...
`.github/workflows/ci.yml` runs on every push to `main` and every pull request targeting `main`, as 9 independent parallel jobs (no job waits on another). Every job gets a least-privilege `contents: read` default via a workflow-level `permissions:` block (only `trivy` needs more, and declares its own job-level override); every third-party action in both `ci.yml` and `docker-image.yml` is pinned to a commit SHA rather than a floating version tag, with a version comment alongside each pin and a `.github/dependabot.yml` (github-actions ecosystem) keeping those pins current (issue #145). The same `dependabot.yml` also tracks the `docker` ecosystem (issue #148), opening update PRs when the Dockerfile's base-image tags (`node:26-alpine`, `nginxinc/nginx-unprivileged:1.31-alpine`) ship a new release; npm is deliberately not tracked there, since production dependency risk is already gated by the `npm-audit` job below. Major-version bumps for `node` are excluded from that automation (issue #193) — Node only ever promotes an even-numbered major to Active LTS, and a routine Dependabot merge previously landed the build stage on a non-LTS release without review, so major bumps now require a deliberate manual PR instead.
...
**`npm audit` gate:** `audit-ci --moderate --skip-dev --allowlist GHSA-qwww-vcr4-c8h2` fails the job (and therefore the PR check) on moderate/high/critical findings in the *production* dependency tree — plain `npm audit` has no per-advisory allowlist, so the gate uses `audit-ci` instead, which adds exactly one: the react-router RSC-mode CSRF advisory this app can't reach (see `SECURITY.md`'s Dependency policy section). Any other finding still fails the gate. The allowlist entry carries a dated `# Allowlist entry added: 2026-07-28` marker (issue #145) so the exception can't silently calcify: `.github/__tests__/ci.test.js` fails once that entry is more than 180 days old, forcing either a fresh re-review or landing the tracked React 19 + react-router 8 upgrade (`docs/ENHANCEMENTS.md`) that removes the need for it. A separate `npm audit --omit=prod` step always runs (even if the gate step failed) and never fails the job itself — its output is appended to the workflow run's own step summary, so *dev*-tree findings (like the 3 moderate Storybook-chain advisories noted in `SECURITY.md`) stay visible without blocking a merge.

**Trivy image scan:** the `trivy` job builds the Docker image (independently of the `docker-build` job) and scans it with [Trivy](https://trivy.dev/) (`aquasecurity/trivy-action`). A gate step fails the job on CRITICAL/HIGH findings that have an available fix (`ignore-unfixed: true` — skips upstream Alpine CVEs with no patch yet, so the gate only trips on things a version bump could actually fix). A second step, which runs even if the gate failed, scans again for every severity including unfixed findings and uploads the result as SARIF to the repository's Security tab (Code Scanning), so lower-severity or currently-unfixable findings stay visible for tracking without ever blocking a merge — the same report-without-blocking posture as the `npm-audit` job's dev-tree step. This `ci.yml` scan is a same-commit *proxy* build, not the artifact that actually ships — see below for the release image's own scan.

**Lighthouse budgets:** `lighthouserc.json` (repo root) drives `@lhci/cli` against a real production build — `npm run build` then `vite preview` (not the dev server, so scores reflect the minified/bundled app that actually ships). `ubuntu-latest` doesn't guarantee a pre-installed Chrome for `lhci` to launch, so the job installs one explicitly (`browser-actions/setup-chrome`) and points `lhci` at it via the `CHROME_PATH` env var. Four routes are scored: the dashboard (`/`), a representative game (`/game/animal-sounds`), the parent analytics dashboard (`/parent`), and the kid-facing progress page (`/my-progress`). All four Lighthouse categories — performance, accessibility, best-practices, SEO — must score at least 0.8 or the job fails; each run's actual report is uploaded to Lighthouse's temporary public storage (link printed in the job log), so a score drifting down toward 0.8 is visible before it ever crosses the line.

**Release image scan (`docker-image.yml`, issue #145):** the image that `docker-image.yml` actually pushes to Docker Hub/GHCR on a published release is built once locally (`push: false`, `load: true`, tagged `playground:release`), Trivy-scanned with the same CRITICAL/HIGH fixable-findings gate as the `ci.yml` `trivy` job, and only then tagged and pushed via plain `docker tag`/`docker push` — replacing the previous flow, which built and pushed twice via `docker/build-push-action` with no scan of the pushed artifact at all. A scan failure now blocks the release.

Every workflow-adjacent config file has its own static structure tests, following the same pattern as the nginx security-header tests — proving the *configuration's shape*, not the runtime behavior (which is proven every time the workflow actually runs): `.github/__tests__/ci.test.js` parses `ci.yml` and asserts its trigger scope, job set, the audit gate's exact flags and allowlist freshness, the `trivy` job's gate/report/upload shape, the workflow-level `permissions:` default, and that every action is SHA-pinned; `.github/__tests__/dockerImage.test.js` asserts `docker-image.yml`'s permissions, SHA-pinning, and the build→scan→tag/push ordering of the release scan gate; `.github/__tests__/dependabot.test.js` asserts the github-actions and docker ecosystems are tracked (and that npm deliberately isn't); `.github/__tests__/lighthouserc.test.js` asserts the 4 routes and 4 category thresholds.
