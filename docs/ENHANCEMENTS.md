...
Items marked **AU-n** come from the 2026-07-12 a11y/i18n/UX audit (`docs/accessibility_usability.md`) and its 2026-08-31 re-audit (`docs/superpowers/specs/2026-08-31-accessibility-usability-audit-findings.md`), which found zero automated-scan violations — these are the judgment-level gaps automation can't see. (The 2026-07-12 audit also verified the memory board's reduced-motion coverage is already complete, so the previous "reduced-motion audit for the memory board" entry is resolved and removed. The 2026-08-31 re-audit re-verified AU-1/AU-2/AU-3/AU-6/AU-8 below as genuinely resolved by direct source inspection, not just this backlog's say-so.)

- **`ResumePrompt` doesn't move focus to its heading (AU-9, Medium)** — every other phase transition in the engine (`GameResults`, `ParentalLockGate`) calls `useFocusOnMount()` on its heading so a keyboard/screen-reader user is told a new screen appeared; `src/components/ResumePrompt.jsx` doesn't, so a session resumed after a crash/close/exit silently swaps the question view for the Resume/Start Fresh prompt with no announcement. One-line fix: mirror `GameResults`'s `useFocusOnMount()` + `tabIndex={-1}` pattern, and add a focus assertion to `ResumePrompt.test.jsx` alongside its existing axe check. Found by the 2026-08-31 re-audit.
- **Full RTL support (`dir` attribute sync)** — the remaining half of RTL readiness (logical CSS properties already shipped in v0.16.0); requires an actual RTL locale to exist before it can be meaningfully verified. (Re-confirmed outstanding by both the 2026-07-12 and 2026-08-31 audits: `lang` syncs, `dir` doesn't.)
...
- ~~**Dependabot for Dockerfile base images**~~ — done (issue #148): `.github/dependabot.yml` now also tracks the `docker` ecosystem at the repo root, opening update PRs when `node:24-alpine` or `nginxinc/nginx-unprivileged:1.30-alpine` ship a new major.minor tag. npm remains deliberately untracked (guarded by a negative test in `.github/__tests__/dependabot.test.js`) — production deps are already gated by `audit-ci` in CI, a separate concern. This satisfies the "revisit if/when Dependabot is added" condition the 2026-07-18 container-hardening spec attached to its major.minor-over-digest base-image pinning choice; digest pinning itself is still a deliberate non-goal (see `docs/superpowers/specs/2026-07-18-container-hardening-design.md`), not something this change reopens on its own.
- ~~**Node LTS for pipelines and codebase**~~ — done (issue #193): `.github/workflows/ci.yml`'s pipelines were already pinned to `node-version: 24` (Active LTS). The Dockerfile's build stage stayed on `node:26-alpine` (accepted deliberately — it reaches Active LTS in October 2026, and re-pinning to 24 now would just mean re-bumping again in six weeks), but the process gap that put it there without review is now closed: `.github/dependabot.yml` no longer auto-opens major-version bump PRs for the `node` docker dependency (Node only ever promotes even-numbered majors to LTS; odd majors never go LTS at all), and a static test (`nginx/__tests__/securityHeaders.test.js`) fails CI if the pinned major ever goes odd. `package.json` also gained an `engines.node` field (`>=24`) declaring the supported floor.
