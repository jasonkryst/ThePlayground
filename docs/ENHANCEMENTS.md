...
- **Parent Dashboard chart-axis labels and heatmap row alignment under large text** — two known residual gaps from the wave-2 px→rem conversion (issue #83): `ParentDashboard.jsx`'s Recharts `tick={{ fontSize: 12 }}` axis labels are a JS prop (not CSS), so they don't scale with OS/browser large-text settings the way the rest of the app now does; and `.heatmap__day-label`'s box can now grow with its (now-resizable) text while `.heatmap__grid`'s fixed-height rows can't, so the two visually desync under a large-text setting (no content clips or is lost — an accepted trade-off, just worth tracking).
