...
let mockSettings = {
  ...
  introDismissed: { 'character-match-bluey': true },
}
...
async function startGameIfIntroIsVisible() {
  const startButton = screen.queryByTestId('game-intro-start')
  if (!startButton) {
    return
  }
...
describe('CharacterMatchGameBluey', () => {
  it('renders a question with a prompt and answer buttons', async () => {
    await act(async () => { render(<CharacterMatchGameBluey onGameEnd={onGameEnd} />) })
    await startGameIfIntroIsVisible()
  ...
  it('renders a decorative image for each answer choice', async () => {
    let container
    await act(async () => { container = render(<CharacterMatchGameBluey onGameEnd={onGameEnd} />).container })
    await startGameIfIntroIsVisible()
    ...
    await act(async () => {
      render(
        <ShellContext.Provider value={{ setGameStatus }}>
          <CharacterMatchGameBluey onGameEnd={onGameEnd} />
...
describe('CharacterMatchGameBluey — how-to-play intro', () => {
  ...
  it('persists introDismissed for this game when "don\'t show again" is checked before starting', async () => {
    ...
    await act(async () => {})
    expect(mockUpdateSetting).toHaveBeenCalledWith('introDismissed', { 'character-match-bluey': true })
  })
