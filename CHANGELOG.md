...
- The in-game header: the streak badge shared the same row as the game title with no `white-space: nowrap`/`flex-shrink: 0`, so on a narrow phone it wrapped its own text ("2" / "in a row!") instead of the title truncating. Moved the streak badge to its own centered row below the title/home-button row (only rendered while a session is active), so it always renders on one line and the title gets the full row's width.
