...
- **CI/CD supply chain (issue #145):** every job in `.github/workflows/ci.yml` gets a least-privilege `contents: read` default via a workflow-level `permissions:` block (`trivy` overrides with the extra `security-events: write` it needs); every third-party GitHub Action referenced in `ci.yml` and `docker-image.yml` is pinned to a commit SHA rather than a floating version tag (a tag can be repointed by the action's maintainer, or an attacker who compromises their account, without this repo's consent), with a version comment alongside each pin and a `.github/dependabot.yml` (github-actions ecosystem, weekly) opening update PRs when a pinned action ships a new release.
