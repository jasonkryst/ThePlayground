...
- **A service worker (issue #96)** precaches the app for offline use. It's generated by `vite-plugin-pwa` from the app's own build output — no hand-written interception logic — and runs same-origin under the existing CSP (`default-src 'self'`) exactly like any other first-party script; it doesn't fetch or execute anything the CSP wouldn't already allow. Scope is the whole origin (`/`), which is standard for an app with no other same-origin content to isolate it from.
...
| Version | Supported |
|---|---|
| Latest release (currently 1.1.x — see [`CHANGELOG.md`](CHANGELOG.md) or the dashboard footer) | ✅ |
