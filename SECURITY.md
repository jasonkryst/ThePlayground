...
- **Official base images, pinned:** `node:26-alpine` (build), `nginxinc/nginx-unprivileged:1.31-alpine` (runtime) — both pinned to a major.minor version rather than a floating tag, for reproducible builds (issue #85). `.github/dependabot.yml` (`docker` ecosystem, weekly, issue #148) opens update PRs when either tag ships a new release, so the pins don't go stale — except node major-version bumps, which are deliberately excluded (issue #193): Node only ever promotes an even-numbered major to Active LTS, so a routine Dependabot merge previously landed the build stage on a non-LTS release (`node:24-alpine` → `node:26-alpine`, 1.1.9) without anyone deciding that was the right time. Node major bumps now require a deliberate manual PR, opened once the target major has actually reached LTS; a static test (`nginx/__tests__/securityHeaders.test.js`) guards that the pinned major stays even in the meantime.
...
- **Automated image vulnerability scanning (SEC-4 remainder, issue #132):** the `trivy` job in `.github/workflows/ci.yml` scans the built image with Trivy on every push/PR. A gate step fails the job on CRITICAL/HIGH findings with an available fix (`ignore-unfixed: true` skips upstream OS-package CVEs with no patch yet, so the gate only trips on actionable findings). A second step, which runs even if the gate failed, scans every severity including unfixed findings and uploads the result as SARIF to this repository's Security tab, so lower-severity or currently-unfixable findings stay visible for tracking without ever blocking a merge — the same posture `npm audit`'s dev-tree report already uses below. **The pushed release image is scanned too (issue #145):** `.github/workflows/docker-image.yml` used to build the image twice more and push straight from each build, so the artifact actually published to Docker Hub/GHCR was never itself scanned — only a same-commit proxy build in `ci.yml` was. It now builds the release image once locally, runs the same CRITICAL/HIGH fixable-findings Trivy gate against it, and only then tags and pushes that exact scanned image; a scan failure blocks the release.
- **Source-level scanning (`.github/workflows/security.yml`):** two additional jobs run on every push/PR to main and on a weekly schedule to catch newly-disclosed CVEs between commits. (1) **CodeQL** (`codeql` job) — GitHub's static application security testing (SAST) tool analyses the JavaScript source for common vulnerability classes (injection, prototype pollution, DOM XSS, etc.) and uploads results as SARIF to the Security → Code scanning tab. This is complementary to the container scan: it operates on source code before a Docker image is ever built. (2) **Trivy filesystem scan** (`trivy-fs` job) — Trivy in `fs` mode scans the project directory (including `node_modules` via the lockfile) for known-vulnerable npm packages. This is a second independent path alongside `npm audit`: both gate on CRITICAL/HIGH fixable findings; both report all-severity findings as SARIF to the Security tab (with `category: trivy-fs` to distinguish from the container scan's SARIF upload). A dedicated test file (`.github/__tests__/security.test.js`) verifies trigger events, permissions, scan modes, gate configuration, and SHA-pinning for both jobs.
- **CI/CD supply chain (issue #145):** every job in `.github/workflows/ci.yml` and `.github/workflows/security.yml` gets a least-privilege `contents: read` default via a workflow-level `permissions:` block (jobs that upload SARIF override with the extra `security-events: write` they need); every third-party GitHub Action referenced in `ci.yml`, `docker-image.yml`, and `security.yml` is pinned to a commit SHA rather than a floating version tag (a tag can be repointed by the action's maintainer, or an attacker who compromises their account, without this repo's consent), with a version comment alongside each pin and a `.github/dependabot.yml` (github-actions ecosystem, weekly) opening update PRs when a pinned action ships a new release.
